import {normalizeProject, resolveLocalPath, createProjectFile} from './projectFiles.js';
...
test('migration retains student source and CSS cascade', () => {
  ...
  expect(p.files['index.html']).toBe('<main>Moja praca</main>');
...
test('migration updates only recognized resource references', () => {
  const p=normalizeProject({html:'<head><link rel="stylesheet" href="base.css"><link rel="stylesheet" href="theme.css"></head><body><script src="script.js" defer></script></body>',js:'const App=()=>null;',baseCss:'',themeCss:''},{track:'react'});
...
test.each(['../x.js','/x.js','https://a/x.js','__proto__','a\\x.js','constructor.js','a/../../b.js'])('rejects unsafe new name %s', name=>{
...
test('resolves relative imports within project and optional extensions',()=>{
  ...
  expect(resolveLocalPath('components/Card.jsx','../App',files)).toBe('App.jsx');
  expect(resolveLocalPath('App.jsx','./components/Card',files)).toBe('components/Card.jsx');
...
test('creates independent file without overwriting others',()=>{
  ...
  const result=createProjectFile(project,{type:'react',name:'components/my-card'});
  expect(result.path).toBe('components/my-card.jsx');
  expect(result.project.files[result.path]).toContain('export default function MyCard');
  expect(project.files).toEqual({'App.jsx':'keep'});
  expect(()=>createProjectFile(project,{type:'react',name:'App.jsx'})).toThrow();
  expect(()=>createProjectFile(project,{type:'css',name:'other.js'})).toThrow();
});
